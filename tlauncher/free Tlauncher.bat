...
:options
    ...
    ECHO 1) Install (Install free Tlauncher on this computer)
    ECHO 2) UnInstall (Remove free Tlauncher from the computer)
...
:install
    ...
    FINDSTR /V "213.32.111.137 auth.tlauncher.org " "%HOSTS_FILE%" > "%TEMP_HOSTS_FILE%"
    REM Add the redirect to the new file
    ECHO 213.32.111.137 auth.tlauncher.org  >> "%TEMP_HOSTS_FILE%"
